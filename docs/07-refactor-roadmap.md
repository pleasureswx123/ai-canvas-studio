...
- [x] 视频首帧/尾帧引用：视频节点可从上游图片中选择首帧和尾帧，并传递给视频 provider。
- [x] 视频剪辑基础能力：视频节点可设置开始/结束秒数，Node API 使用本地 ffmpeg 输出新工程视频资产。
- [x] 历史面板：按工程展示最近 100 条图片/视频生成历史，可拖回画布复用。
- [x] 工程封面/缩略图生成：保存和列表接口自动推导封面，左侧工程列表展示缩略图。
...
- [x] 编写 `MIGRATION.md`：旧项目数据迁移步骤、备份策略、失败恢复。
- [x] 编写 `OPERATIONS.md`：环境变量、端口、启动、日志、常见错误。
- [x] 编写 `CONTRIBUTING.md`：目录规范、分支规范、提交规范、代码评审关注点。
- [x] 完成旧项目功能对照表：已迁移、替代方案、废弃原因。见 `docs/10-legacy-feature-matrix.md`。
- [x] 完成真实项目迁移演练，记录耗时和问题。已对旧工程 `proj_mo57tkma_e9n870` 执行 preview，结果记录在 `MIGRATION.md`。
- [x] 锁定旧项目为只读 reference。已在 README、迁移文档、贡献规范和旧功能对照表中明确。
- [x] 新项目作为默认开发入口。已在 README 和 CONTRIBUTING 中明确。
- [x] 验收：团队成员只看新项目文档即可启动、开发、调试、迁移旧数据。
...
目标：当前暂不进入部署阶段，优先把产品变得更好用、更易用、更清晰。

- [ ] 扩展 Playwright 产品路径：上传图片、拖入素材、图片生成、视频任务轮询。
- [ ] 增加真实 provider smoke 脚本：读取 `.env.local`，按已启用 provider 输出验收报告。
- [ ] 打磨节点操作体验：选中态、复制、删除、多选、快捷操作和布局反馈。
- [ ] 打磨生成任务体验：任务队列、失败重试、错误解释、结果回填提示。
- [ ] 打磨素材库体验：批量管理、分类维护、搜索过滤、缺失素材恢复。
- [x] 杜绝浏览器原生 `prompt`、`confirm`、`alert`：新建、重命名、素材审核改为应用内弹窗，并加入 `npm run ui:check` 门禁。
- [x] 增加小白用户使用手册：说明系统用途、解决问题、核心概念、完整操作步骤和常见问题。
- [x] 增加 LibTV 对标与专业视频创作差距文档：明确当前项目不是专业级成品，后续必须围绕脚本、分镜、主体一致性和导出补齐。
- [ ] 部署阶段再实现 Docker、Nginx、数据卷、一键首次部署和一键更新脚本。
- [ ] 任意需求变化必须同步更新 `AGENT.md`、README 和相关 docs。
